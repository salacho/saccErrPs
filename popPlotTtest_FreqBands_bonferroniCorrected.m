function popPlotTtest_FreqBands_bonferroniCorrected(expVarFreq,pValsFreq,errDiffFreqTxt,ErrorInfo)
% function popPlotTtest_FreqBands_bonferroniCorrected(expVarFreq,pValsFreq,errDiffFreqTxt,ErrorInfo)
%
%
%
...
for iArray = 1:length(plotInfo.arrayLoc)
    ...
    for iClim = 0:1
        ...
        hold on 
        title(sprintf('Residuals with p-val < %0.03f bonferroni',ErrorInfo.analysis.ANOVA.pValCrit) ,'FontSize',plotInfo.axisFontSz), axis off                                                                % remove axis and background
        
        % Saving figures
        if plotInfo.savePlot
            saveFilename = sprintf('%s-ErrDiff-Ttest_freqBand%s-%s-clim%i-pval%0.03f%s-bonferroni.png',infoStr.strPrefix,...
    ...
    %set(hLeg,'box','off','fontweight','bold','color',[1 1 1]),
    hold on
    title(sprintf('pVals<%0.03f bonferroni',ErrorInfo.analysis.ANOVA.pValCrit),'FontSize',plotInfo.axisFontSz-1), axis off                                                                % remove axis and background
    
    % Saving figures
    if plotInfo.savePlot
        saveFilename = sprintf('%s-ErrDiff-Ttest_freqBand%s-%s-onlyPval%0.03f%s-bonferroni.png',infoStr.strPrefix,...
