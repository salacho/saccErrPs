...
for iArray = 1:length(plotInfo.arrayLoc)
    ...
    for iClim = 0:1
        ...
        if plotInfo.savePlot
            saveFilename = sprintf('%s-ErrDiff-Ttest_freqBand%s-%s-clim%i-pval%0.2f%s-bonferroni.png',infoStr.strPrefix,...
    ...
    % Saving figures
    if plotInfo.savePlot
        saveFilename = sprintf('%s-ErrDiff-Ttest_freqBand%s-%s-onlyPval%0.2f%s-bonferroni.png',infoStr.strPrefix,...
