...
hPlot = imagesc((squeeze(expVarMod)).*(pVals <= ErrorInfo.analysis.ANOVA.pValCrit));
...
xlabel('Time from feedback onset [s]','FontSize',plotParams.axisFontSize+2,'FontWeight',plotParams.axisFontWeight)
...
if ErrorInfo.plotInfo.savePlot
    if strcmp(ErrorInfo.session(1),'p')
        saveFilename = sprintf('%s-epochsExpVar-[%i-%ims]-[%0.1f-%iHz].png',fullfile(ErrorInfo.dirs.saveFilename,ErrorInfo.session),...
