function plotDist2Tgt(tgt2DistEpochs,tgtErrRPs,corrEpochs,ErrorInfo)
% function plotDist2Tgt(tgt2DistEpochs,tgtErrRPs,corrEpochs,ErrorInfo)
% 
% Plots distance to target traces per array and per location in visual
% field (6 target location). Mean array and mean epochs.
%
% INPUT
...
[~,~,incorrMeanDist2TgtArray,incorrStdDist2TgtArray,ErrorInfo,meanDist2Tgt,stdDist2Tgt] = ...
    getMeanDist2TgtErrPs(tgt2DistEpochs,ErrorInfo);
...
[corrMeanTgt,~,~,~,corrMeanTgtArray,incorrMeanTgtArray,~,~] = getMeanTgtErrPs(tgtErrRPs,ErrorInfo);
    ...
    for iArray = 1:length(arrayLoc)
        ...
        for iTgt = 1:nTgts
            ...
            if ErrorInfo.epochInfo.nIncorrEpochsTgt(iTgt) ~= 0
                ...
                % Figure properties
                hFig(iTgt,iArray) = figure;
                set(hFig(iTgt,iArray),'PaperPositionMode','auto','Position',[1281 1 1280 948],...
                    'name',sprintf('%s mean error - all distance to true location - tgt%i in %s array %s',ErrorInfo.session,iTgt,arrayLoc{iArray},infoStr.yLimTxt),...
                ...
                for iDist = 1:length(distVals)
                    ...
                    for ii = 1:length(ErrorInfo.plotInfo.arrayChs(iArray,:))
                        iCh = ErrorInfo.plotInfo.arrayChs(iArray,ii);
                        ...
                        hold on,
                        if iDist == 1
                            hPlot(1,subCh) = plot(timeVector,corrEpochTgt,'Color',plotInfo.distColors(1,:),'LineWidth',plotInfo.lineWidth);       %#ok<AGROW>
                        end
                        hPlot(iDist+1,subCh) = plot(timeVector,meanIncorrEpoch(iCh,:),'Color',plotInfo.distColors(iDist+1,:),'LineWidth',plotInfo.lineWidth); %#ok<AGROW>
                        axis tight
...
%% Plot dist2Tgt per channel regardless of target location
...
for iArray = 1:length(arrayLoc)
    ...
    for iCh = plotInfo.arrayChs(iArray,1):plotInfo.arrayChs(iArray,end)
        ...
        plotErrorBars(timeVector,meanCorr(iCh,:),meanCorr(iArray,:)-stdCorr(iCh,:),meanCorr(iCh,:)+stdCorr(iCh,:),plotInfo);               % Blue for correct epochs
...
%% Plot dist2Tgt per array regardless of target location
hFig = figure;
set(hFig,'PaperPositionMode','auto','Position',[697         266        1841         463],...
    'name',sprintf('%s mean dist2Tgt for %s',ErrorInfo.session,arrayLoc{iArray}),...
    'NumberTitle','off','Visible',plotInfo.visible);

% All dist2Tgt regardless of trial and true target location
[dist1Epochs,dist2Epochs,dist3Epochs,~] = getDist2Tgt(tgt2DistEpochs,ErrorInfo); %#ok<*ASGLU>

for iArray = 1:length(plotInfo.arrayLoc)
    % Number of channels for this array
    nChs = length(plotInfo.arrayChs(iArray,:));
    [~,nCorrTrials,nSamps] = size(corrEpochs);
    % Mean and st.dev. of correct trials
    reshapeCorrEpochs = reshape(corrEpochs(plotInfo.arrayChs(iArray,:),:,:),[nChs*nCorrTrials nSamps]);
    meanCorr = nanmean(reshapeCorrEpochs,1);
    stdCorr = nanstd(reshapeCorrEpochs,[],1);
    ...
    % Plot mean epochs per channel for correct trials
    plotInfo.plotColors(1,:) = plotInfo.distColors(1,:);
    hCorr = plotErrorBars(timeVector,meanCorr,meanCorr - stdCorr,meanCorr + stdCorr,plotInfo);               % Blue for correct epochs
    
    % dist2Tgt
    legendTxt{1} = 'Correct';                                           % legend for correct trials
    for iDist = 1:3
        eval(sprintf('data2Plot = dist%iEpochs;',iDist));
        [~,nTrials,nSamps] = size(data2Plot); %#ok<NODEF>
        reData2Plot = reshape(data2Plot(plotInfo.arrayChs(iArray,:),:,:),[nChs*nTrials nSamps]);
        meanData = nanmean(reData2Plot,1);
        stdData = nanstd(reData2Plot,[],1);
        
        % Dist2Tgt
        plotInfo.plotColors(1,:) = plotInfo.distColors(1+iDist,:);
        hIncorr(iDist) = plotErrorBars(timeVector,meanData',meanData - stdData,meanData + stdData,plotInfo);
        
        % Legend
        legendTxt{1+iDist} = sprintf('Dist %i',iDist);
    end
    axis tight
    hSession = plot(0,0,'k','lineWidth',plotInfo.lineWidth); legendTxt{5} = sprintf('%s-%s',ErrorInfo.session,char(arrayLoc(iArray)));
    legend([hCorr.H [hIncorr(:).H] hSession],legendTxt,0,'FontSize',plotInfo.axisFontSz+1,'FontWeight','Bold','location','southwest')                                            % Include legend
    ...
    axis tight;
    title(sprintf('%s',ErrorInfo.plotInfo.arrayLoc{iArray}),'FontSize',plotInfo.titleFontSz,'FontWeight',plotInfo.titleFontWeight)    
end

% Save plot
if ErrorInfo.plotInfo.savePlot
    saveFilename = sprintf('%s-corrIncorr-meanArrayDist2Tgt%s%s.png',infoStr.strPrefix,...
        infoStr.signProcStr,infoStr.strSuffix);
    saveas(hFig,saveFilename)
end
