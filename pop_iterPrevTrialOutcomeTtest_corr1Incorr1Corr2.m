function pop_iterPrevTrialOutcomeTtest_corr1Incorr1Corr2(subject)
% function pop_iterPrevTrialOutcomeTtest_corr1Incorr1Corr2(subject)
%
%
%
...
if popErrorInfo.analysis.balanced
    ...
    nCorr = nan(2,nIter);
    % Incorrect
    expVarIncorr = nan(size(corrEpochsErrPrev,1),nIter,size(corrEpochsErrPrev,3));
    pValsIncorr  = nan(size(corrEpochsErrPrev,1),nIter,size(corrEpochsErrPrev,3));
    FIncorr      = nan(size(corrEpochsErrPrev,1),nIter,size(corrEpochsErrPrev,3));
    muIncorr     = nan(size(corrEpochsErrPrev,1),2,size(corrEpochsErrPrev,3),nIter);
    nIncorr = nan(2,nIter);
    
    % Iterate
    for iIter =1:nIter
        fprintf('T-test for previous trial outcome for iter %i...\n',iIter)
        [expVarCorr(:,iIter,:),nCorr(:,iIter),pValsCorr(:,iIter,:),muCorr(:,:,:,iIter),FCorr(:,iIter,:),popErrorInfo] = getEpochsExpVar(corrEpochsCorrPrev,incorrEpochsCorrPrev,popErrorInfo);
        [expVarIncorr(:,iIter,:),nIncorr(:,iIter),pValsIncorr(:,iIter,:),muIncorr(:,:,:,iIter),FIncorr(:,iIter,:),popErrorInfo] = getEpochsExpVar(corrEpochsErrPrev,incorrEpochsErrPrev,popErrorInfo);
    end
else
    % Correct
    [expVarCorr,nCorr,pValsCorr,muCorr,FCorr,popErrorInfo] = getEpochsExpVar(corrEpochsCorrPrev,incorrEpochsCorrPrev,popErrorInfo);
    % Incorrect
    [expVarIncorr,nIncorr,pValsIncorr,muIncorr,FIncorr,popErrorInfo] = getEpochsExpVar(corrEpochsErrPrev,incorrEpochsErrPrev,popErrorInfo);
end

saveFilename = fullfile(dirs.DataIn,'popAnalysis',sprintf('pop%s-%s-%i_iterPrevTrialOutcome_corr1Incorr1Corr2_Ttest.mat',sessionList{1},sessionList{end}(7:end),numel(sessionList)));
