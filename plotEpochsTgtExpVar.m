...
% X and Y axis values
timeVals = (1:1:size(expVarTgt,3));             % x vals for imagfesc
...
for iNorm = 0:1
    ...
    for iTgt = 1: ErrorInfo.epochInfo.nTgts
        ...
        if iNorm == 0
            hPlot = imagesc((squeeze(expVarTgt(iTgt,:,:))));            % not normalized for all targets exp. variance
            saveTxt = 'nonNorm';
        else
            hPlot = imagesc((squeeze(expVarTgt(iTgt,:,:)))/max(max(max(expVarTgt))),[0 1])%max(max(max(expVarTgt)))]);      % normalized all targets exp. variance
            saveTxt = 'Norm';
        end
...
%         end
    ...
    if ErrorInfo.plotInfo.savePlot
        saveFilename = sprintf('%s-epochsExpVarTgts-%s-[%i-%ims]-[%0.1f-%iHz].png',fullfile(ErrorInfo.dirs.DataOut,ErrorInfo.session,ErrorInfo.session),...
