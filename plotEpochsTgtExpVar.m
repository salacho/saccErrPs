...
for iNorm = 0:1
    ...
    for iTgt = 1: ErrorInfo.epochInfo.nTgts
        ...
        if iNorm == 0
            hPlot = imagesc(squeeze(expVarTgtMod(iTgt,:,:).*(pValsTgt(iTgt,:,:) <= ErrorInfo.analysis.ANOVA.pValCrit)));            % not normalized for all targets exp. variance
            saveTxt = 'nonNorm';
        else
            hPlot = imagesc(squeeze(expVarTgtMod(iTgt,:,:).*(pValsTgt(iTgt,:,:) <= ErrorInfo.analysis.ANOVA.pValCrit))/maxExpVarTgt,[0 1]); %max(max(max(expVarTgtMod)))]);      % normalized all targets exp. variance
            saveTxt = 'Norm';
        end
...
%         end
    ...
    if ErrorInfo.plotInfo.savePlot
        if strcmp(ErrorInfo.session(1),'p')
            saveFilename = sprintf('%s-epochsExpVarTgts-%s-[%i-%ims]-[%0.1f-%iHz].png',fullfile(ErrorInfo.dirs.saveFilename,ErrorInfo.session),...
