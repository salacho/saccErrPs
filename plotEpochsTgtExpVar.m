...
for iNorm = 0:1
    ...
    for iTgt = 1: ErrorInfo.epochInfo.nTgts
        ...
        if iNorm == 0
            hPlot = imagesc(squeeze(expVarTgtMod(iTgt,:,:).*(pValsTgtMod(iTgt,:,:) <= ErrorInfo.analysis.ANOVA.pValCrit/(size(pValsTgtMod,3)*96))));            % not normalized for all targets exp. variance
            %hPlot = imagesc((squeeze(pValsTgtMod(iTgt,:,:)) <= ErrorInfo.analysis.ANOVA.pValCrit/(120*96)));            % not normalized for all targets exp. variance
            saveTxt = 'nonNorm';
        else
            hPlot = imagesc(squeeze(expVarTgtMod(iTgt,:,:).*(pValsTgtMod(iTgt,:,:) <= ErrorInfo.analysis.ANOVA.pValCrit/(size(pValsTgtMod,3)*96)))/maxExpVarTgt,[0 1]); %max(max(max(expVarTgtMod)))]);      % normalized all targets exp. variance
            %hPlot = imagesc(pValsTgtMod(iTgt,:,:) <= ErrorInfo.analysis.ANOVA.pValCrit/(120*96)/maxExpVarTgt,[0 1]); %max(max(max(expVarTgtMod)))]);      % normalized all targets exp. variance
            saveTxt = 'Norm';
        end
