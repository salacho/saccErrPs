function plotMeanSpecgram(corrSpec,incorrSpec,ErrorInfo)
...
%     
    ...
    for iArray = 1:length(plotInfo.arrayLoc)
        ...
        for iClim = 0:1
            ...
            data2plot = squeeze(meanIncorrSpec(tStartIndx:tEndIndx,fStartIndx:fEndIndx,32))' - squeeze(meanCorrSpec(tStartIndx:tEndIndx,fStartIndx:fEndIndx,32))';
            ...
            hold on
            imagesc(time2plot,freq2plot,ones(2,2))
            
            % Saving figures
            if plotInfo.savePlot
                saveFilename = sprintf('%s-ErrDiff-meanEpochSpectrog%s-%s-clim%i-%s%s.png',infoStr.strPrefix,...
                    infoStr.strSpecTrans,plotInfo.arrayLoc{iArray},iClim,infoStr.strSpecRange,infoStr.strSuffix);
                saveas(hFig,saveFilename)
                close(hFig)
            end
        end
