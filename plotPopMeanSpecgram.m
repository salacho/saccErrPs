...
for iFreq = 1:size(plotInfo.specgram.freqs,1)
    ...
    for iArray = 1:length(plotInfo.arrayLoc)
        ...
        % Corr
        climCorr(1,iFreq,iArray) =  min(reshape(meanCorrSpec(:,fStartIndx:fEndIndx,plotInfo.arrayChs(iArray,1):plotInfo.arrayChs(iArray,end)),...
            [length(timeVector)*(fEndIndx-fStartIndx+1)*nArrayChs 1]));
        climCorr(2,iFreq,iArray) = max(reshape(meanCorrSpec(:,fStartIndx:fEndIndx,plotInfo.arrayChs(iArray,1):plotInfo.arrayChs(iArray,end)),...
            [length(timeVector)*(fEndIndx-fStartIndx+1)*nArrayChs 1]));
        % Incorr
        climIncorr(1,iFreq,iArray) =  min(reshape(meanIncorrSpec(:,fStartIndx:fEndIndx,plotInfo.arrayChs(iArray,1):plotInfo.arrayChs(iArray,end)),...
            [length(timeVector)*(fEndIndx-fStartIndx+1)*nArrayChs 1]));
        climIncorr(2,iFreq,iArray) = max(reshape(meanIncorrSpec(:,fStartIndx:fEndIndx,plotInfo.arrayChs(iArray,1):plotInfo.arrayChs(iArray,end)),...
            [length(timeVector)*(fEndIndx-fStartIndx+1)*nArrayChs 1]));
        % ErrDiff
        data2plot = squeeze(meanIncorrSpec(tStartIndx:tEndIndx,fStartIndx:fEndIndx,plotInfo.arrayChs(iArray,1):plotInfo.arrayChs(iArray,end))) - squeeze(meanCorrSpec(tStartIndx:tEndIndx,fStartIndx:fEndIndx,plotInfo.arrayChs(iArray,1):plotInfo.arrayChs(iArray,end)));
        climErrDiff(:,iFreq,iArray) = [nanmin(nanmin(nanmin(data2plot))),nanmax(nanmax(nanmax(data2plot)))];
    end
    ...
    for iArray = 1:length(plotInfo.arrayLoc)
        ...
        for iClim = 0:1
            if ErrorInfo.plotInfo.specgram.doCorr
                for iOut = 1:length(outcomStr)
                    ...
                    for iCh = plotInfo.arrayChs(iArray,1):plotInfo.arrayChs(iArray,end)
                        ...
                        eval(sprintf('climVal = clim%s(:,iFreq,iArray);',outcomStr{iOut}));
                    ...
                    hLeg = legend(legPlots,{[char(plotInfo.arrayLoc(iArray)),infoStr.strSpecTrans],outcomStr{iOut},ErrorInfo.session},0,'location','north');
                    set(hLeg,'box','off','fontsize',plotInfo.axisFontSz)
                    ...
                    if plotInfo.savePlot
                        saveFilename = sprintf('%s-%s-meanEpochSpectrog%s-%s-clim%i-%s%s.png',infoStr.strPrefix,...
                            outcomStr{iOut},infoStr.strSpecTrans,plotInfo.arrayLoc{iArray},iClim,infoStr.strSpecRange,infoStr.strSuffix);
            ...
            climVal = climErrDiff(:,iFreq,iArray);
            ...
            subplot(plotInfo.layout.rows,plotInfo.layout.colms,6)                             % use subplot to place legend outside the graph
            legColorbar(1) = imagesc(climVal);%,[climVal(1,iArray) climVal(2,iArray)])
            hColor = colorbar; hLeg = legend(legColorbar,sprintf('%s-clim%i',ErrorInfo.plotInfo.specgram.transfType,iClim),0,'location','northoutside');
            set(hLeg,'box','off','fontweight','bold'), axis off                                                                % remove axis and background
            hold on
